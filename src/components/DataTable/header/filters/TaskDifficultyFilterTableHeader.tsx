...
function TaskDifficultyFilterTableHeader({ column }: { column: any }) {
    ...
    const difficulties = [1, 2, 3, 4, 5, "N/A"];
    ...
    return (
        <FilterWithClear
            ...
            onChange={(newValue: string) =>
                setSelectedDifficulties(
                    newValue.split(", ").map((v) => (isNaN(Number(v)) ? v : Number(v)))
                )
            }
            onClear={clearFilter}
                    ...
                    {difficulties.map((difficulty) => (
                        ...
                        >
                            {typeof difficulty === "number" ? (
                                <DifficultyBadge level={difficulty as 1 | 2 | 3 | 4 | 5} />
                            ) : (
...
function formatDifficulties(difficulties: (number | string)[]): string {
    const numberDifficulties = difficulties
